...
| `HarnessMcpConnection.timeoutSec`                                                                       | `packages/mcp/src/harness-config.ts`        | Read by the config emitters, set by nothing; the conformance runtime's Codex launch hard-codes `tool_timeout_sec=120` beside `DEFAULT_TIMEOUT_SEC`.                  | Set it from one constant on the connection the runtime builds, or remove it from the connection.                                                                            |
