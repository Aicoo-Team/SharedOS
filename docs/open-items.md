...
| `PolicySource`                                                                                          | not implemented                             | ADR 0020 names it so a synchronous `HostCeiling` can reach policy that lives in a database, loaded once per turn beside the grant set. `HostCeiling` landed without it, and a host closes over state it refreshes on its own schedule instead. | Move the per-turn load into the kernel and give `narrow` its `policy` argument, or drop the port from ADR 0020 and say a ceiling closes over its own state.                 |
