...
Codes from `@aicoo/sharedos-adapters`. The `harness_*` and `model_call_failed`
codes are how a driver or plugin ends its turn, so they surface as a `failed`
...
| `turn_cancelled`                 | cancelled | The MCP harness runtime's signal fired before or while the CLI ran — the envelope's code, reported by the plugin                                                                                    |
