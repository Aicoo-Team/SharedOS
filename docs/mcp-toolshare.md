...
| `SHAREDOS_MODEL_AUTH`                                                  | `api-key` (default) or `codex-subscription`, which authenticates from the login `codex login` stored and needs no key; an absent login is reported the way an absent binary is (ADR 0019)                                                                                                                                                                                                                                                                                |
...
| `SHAREDOS_MODEL`, `SHAREDOS_MODEL_BASE_URL`, `SHAREDOS_MODEL_PROVIDER` | The model column's model (default `DSH_MODEL`, else the config's `model.id`, else `deepseek-v4-flash`), API root (default `https://api.deepseek.com`), and provider label (default the config's `model.provider`, else `deepseek`). On the Responses wire the defaults are the config's `model.id` else `gpt-5-codex`, `https://chatgpt.com/backend-api/codex` under a subscription (else `https://api.openai.com/v1`), and `openai`; `DSH_MODEL` is not consulted there |
