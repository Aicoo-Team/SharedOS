...
| `SHAREDOS_MODEL_AUTH`                                                  | `api-key` (default) or `codex-subscription`, which authenticates from the login `codex login` stored and needs no key; an absent login is reported the way an absent binary is (ADR 0019). `pnpm login:subscription` obtains one by device code, with no vendor CLI installed (ADR 0020)                                                                                                                                                                             |
