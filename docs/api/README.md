...
This reference is generated from the twelve public SharedOS package entry points.
...
| `@aicoo/sharedos-precedent`   | Admission rules for an auto-decision proposed from precedent   |
| `@aicoo/sharedos-os`          | Standard permission-controlled `files` and `repo` tools        |
...
| `@aicoo/sharedos-adapters`    | The model driver, and the MCP harness runtime for vendor CLIs  |
