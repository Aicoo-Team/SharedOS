    ...
    attempts: [
      {
        ...
        role: "attack",
        description:
          "Read the record behind a granted view, and receive it whole instead of the view.",
        tool: READ_TOOL,
        toolArguments: { path: [...VIEWED_FILE] },
        expect: { statuses: ["denied"], reasonCodes: ["view_required"] },
      },
      {
        id: "read-the-declared-view",
        role: "control",
        description:
          "Read the granted view by name. The kernel serves the projection -- only the view's declared fields -- in place of the record.",
        tool: READ_TOOL,
        toolArguments: { path: [...VIEWED_FILE], view: VIEW_NAME },
        expect: { statuses: ["succeeded"] },
      },
      {
        id: "name-the-view-elsewhere",
        role: "probe",
        description:
          "Name the granted view against a record no capability serves a view of. A view's name is not authority, and carries none to other resources.",
        tool: READ_TOOL,
        toolArguments: { path: [...READ_ONLY_FILE], view: VIEW_NAME },
